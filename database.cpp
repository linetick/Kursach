...
bool DataBase::addDelivery(int Price)
{
    ...
    QSqlQuery query(db);
    query.prepare("INSERT INTO Доставка (ID_Покупателя, Общая_Цена, Дата_Покупки, Дата_Доставки, Статус) VALUES (:customerId, :totalPrice, :purchaseDate, :deliveryDate, :status)");
    ...
    query.bindValue(":deliveryDate", "В обработке");
    query.bindValue(":status", "В обработке");

    if (!query.exec()) {
                qDebug() << "Ошибка вставки в таблицу Доставка:" << query.lastError().text();
                            db.rollback();
                return false;
    }
    ...
    for (const auto& item : cart)
        {
            ...
            query.prepare("INSERT INTO Заказ (ID_Доставки, ID_Продукта, Цена, Количество) VALUES (:deliveryId, :productId, :price, :quantity)");
            query.bindValue(":deliveryId", deliveryId);
            query.bindValue(":productId", productId);
...
QSqlQueryModel* DataBase::getDeliveryInfo(int a) {
    ...
    if(a == 5)
    {
            ...
            SELECT
                ...
                Люди.Имя,
                Люди.Отчество,
                Люди.Телефон AS Номер_телефона,
                Люди.Адрес,
                Доставка.Статус,
...
void DataBase::getOrdersByDeliveryId(int deliveryId) {
        ...
        modell->removeColumn(1); // Убираем колонку ID_Доставки
